...
def test(test_df, model_path, id2label, label2id):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
       model_path, trust_remote_code=True, num_labels=len(label2id), id2label=id2label, label2id=label2id, torch_dtype="auto", device_map="auto", quantization_config=bnb_config if "deberta" not in model_path.lower() else None
    )
...
if __name__ == '__main__':
    ...
    dev_silver = pd.read_json('dev_silver.jsonl', lines=True)
    dev_silver = dev_silver[dev_silver.conspiracy != "unknown"].reset_index(drop=True)
    test_silver = pd.read_json('test_silver.jsonl', lines=True)
    test_silver = test_silver[test_silver.conspiracy != "unknown"].reset_index(drop=True)
    print(dev_silver.info())
    print(dev_silver.conspiracy.value_counts())
    print(test_silver.info())
    print(test_silver.conspiracy.value_counts())
    
    train_df = pd.concat([train_df, dev_silver, test_silver], ignore_index=True).drop_duplicates(subset="text").sample(frac=1., random_state = 0).reset_index(drop=True)
    ...
    if args.test_only != True:
      fine_tune(train_df, valid_df, f"models/{model}-robust", id2label, label2id, model, args.continue_train)

    # test detector model
    results, predictions, probs = test(test_df, f"models/{model}-robust/best/", id2label, label2id)
